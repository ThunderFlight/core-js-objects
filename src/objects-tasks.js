...
function mergeObjects(objects) {
  ...
  objects.forEach((item) => {
    Object.entries(item).forEach((elements) => {
      const [key, value] = elements;
      if (Object.keys(concatObject).includes(key)) {
        concatObject[key] += value;
      } else {
        concatObject[key] = value;
      }
    });
...
function compareObjects(obj1, obj2) {
  ...
  Object.keys(obj1).forEach((key) => {
    if (obj1[key] !== obj2[key]) bool = false;
  });
  return bool;
}
...
function makeWord(lettersObject) {
  ...
  const emptyArray = Array.from({ length: index }, (v, i) => i);

  Object.entries(lettersObject).forEach((entries) => {
    const [key, value] = entries;
    value.forEach((val) => {
      emptyArray.splice(val, 0, key);
    });
  });
  return emptyArray.filter((item) => typeof item === 'string').join('');
}
...
function sellTickets(queue) {
  ...
  const copy = queue;
  const lastElement = queue.length - 1;
  const customer = queue[lastElement];
  copy.splice(lastElement, 1);
  const sum = copy.reduce(
    (accumulator, currentValue) => accumulator + currentValue,
    0
  );
  return sum - customer === 0;
}
